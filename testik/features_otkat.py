...
# def dropping_zeroes(make_count, carsJson4test):
#     newCarsJson = {'ZeroItem':0, 'ZeroItemTwo':0,}
#     new_maker_counter = []
#     index = 0
#     for i in make_count:
#         if make_count[index] != 0:
#            new_maker_counter.append(i)
#            # make_count[i/index]
#            carsJson4test[index] = newCarsJson[index]
#         index += 1
#     newCarsJson.pop('ZeroItem')
#     newCarsJson.pop('ZeroItemTwo')
#     return new_maker_counter, newCarsJson


def dropping_zeroes(make_count, carsJson4test):
    newCarsJson = {}  # Initialize an empty dictionary
    new_maker_counter = []
#
# # Create an iterator for the carsJson4test keys
    brands_iter = iter(carsJson4test.keys())

    for count in make_count:
        brand = next(brands_iter, None)  # Get the next brand
        if brand and count != 0:  # Check if brand exists and count is non-zero
            new_maker_counter.append(count)
            newCarsJson[brand] = carsJson4test[brand]

    return new_maker_counter, newCarsJson


def get_models_for_make(x_tmp):
    models = []
    for i in x_tmp:
...
#
def delete_makes_with_zero(data, makeCount):
...
# newCarsJson = delete_makes_with_zero(carsJson4test,graphing_makes(carsJson4test) )
# print(graphing_makes(newCarsJson))

x,y = dropping_zeroes(graphing_makes(carsJson), carsJson4test)
z = list(y.values())
y = list(y.keys())
def addlabels(x,y):
