...
def get_final_x_y_fromWholeData(JsonStructModels):
    newCarsJson = delete_makes_with_zero(JsonStructModels, graphing_makes(JsonStructModels))
    newModelsWithoutZero = list(newCarsJson.keys())
    return getting_graph_for_makes(newModelsWithoutZero, get_models_for_make(newModelsWithoutZero))
...
def getDatForUserMakeReqPreReadyToGraph(name_of_req_model):
    models_for_request = get_models_for_make([f'{name_of_req_model}'])
    models_ready_for_x = []
    models_ready_for_y = []
    for i in models_for_request[0]:
        # print('Model' + i)
        try:
            counter = final_filter(name_of_req_model, i)
            if counter > 0:
...
def userRequest(request_usr):
    if request_usr == 'All':
        x_all_makes, y_all_makes = get_final_x_y_fromWholeData(carsJson4test)
